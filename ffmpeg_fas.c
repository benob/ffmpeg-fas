...
static fas_error_type   private_seek_to_nearest_key_by_time (fas_context_ref_type context, double time, int offset);
    ...
    while (FAS_TRUE)
    {
        ...
        if (packet.stream_index == context->stream_idx)
        {
            ...
            if (frameFinished)
            {
                /* compute frame time */
                double pts = 0;
                if(packet.dts == AV_NOPTS_VALUE && context->frame_buffer->opaque && *(uint64_t*)context->frame_buffer->opaque != AV_NOPTS_VALUE) {
                    pts = *(uint64_t *)context->frame_buffer->opaque;
                } else if(packet.dts != AV_NOPTS_VALUE) {
                    pts = packet.dts;
                } else {
                    pts = 0;
                }
                pts *= av_q2d(context->format_context->streams[context->stream_idx]->time_base);
                context->current_frame_time = pts;
                ...
                if (context->frame_buffer->key_frame)
                {
                    ...
                    entry.time = context->current_frame_time;
...
double fas_get_frame_time (fas_context_ref_type context)
{
    if (NULL == context)
        return private_show_error ("NULL context provided for fas_get_frame_index()", FAS_INVALID_ARGUMENT);
    ...
    return context->current_frame_time;
}
...
seek_table_type fas_get_seek_table (fas_context_ref_type context)
{
    ...
    private_complete_seek_table(context);
...
/* fas_seek_to_time */
fas_error_type fas_seek_to_time (fas_context_ref_type context, double time)
{
    ...
    //  printf("seeking to %d (from %d)!\n", target_index, context->current_frame_index);
    if (time == context->current_frame_time)
        return FAS_SUCCESS;

    fas_error = fas_seek_to_nearest_key_by_time (context, time); 
    //fprintf(stderr, "key: %d %f\n", context->current_frame_index, context->current_frame_time);

    if (fas_error != FAS_SUCCESS)
        return private_show_error ("error advancing to key frame before seek", fas_error);

    if (fas_get_frame_time(context) > time)
        return private_show_error ("error advancing to key frame before seek (index isn't right)", fas_error);

    // TODO: yield previous frame if closer to time
    while (fas_get_frame_time(context) < time)
    {
        //fprintf(stderr, "  seek: %d %f\n", context->current_frame_index, context->current_frame_time);
        if (fas_frame_available(context))
            fas_step_forward(context);
        else
            return private_show_error ("error advancing to request frame (probably out of range)", FAS_SEEK_ERROR);
...
/* fas_seek_to_nearest_key_by_time */

fas_error_type fas_seek_to_nearest_key_by_time (fas_context_ref_type context, double time)
{
    return private_seek_to_nearest_key_by_time(context, time, 0);
}

/* private_seek_to_nearest_key_by_time */

fas_error_type private_seek_to_nearest_key_by_time (fas_context_ref_type context, double time, int offset)
{
    if ((NULL == context) || (FAS_TRUE != context->is_video_active))
        return private_show_error ("invalid or unopened context", FAS_INVALID_ARGUMENT);
    ...
    fas_error_type fas_error;
    seek_entry_type seek_entry;
    seek_error_type seek_error = seek_get_nearest_entry_by_time (&(context->seek_table), &seek_entry, time, offset);
    ...
    if (fas_error != FAS_SUCCESS || !context->is_frame_available)  
    {
        // something bad has happened, try previous keyframe
        private_show_warning("processing of seeked keyframe failed, trying previous keyframe");
        return private_seek_to_nearest_key_by_time(context, time, offset + 1);
    }
    ...
    context->current_frame_index = seek_entry.display_index;
    context->current_frame_time = seek_entry.time;
...
fas_error_type private_convert_to_rgb (fas_context_ref_type ctx)
{
    ...
    struct SwsContext* convert_context = sws_getContext(ctx->codec_context->width, ctx->codec_context->height, ctx->codec_context->pix_fmt, \
            ctx->codec_context->width, ctx->codec_context->height, fmt, SWS_BILINEAR, NULL, NULL, NULL);
    if(convert_context == NULL) private_show_error("error converting to rgb", FAS_DECODING_ERROR);

    sws_scale(convert_context, ctx->frame_buffer->data, ctx->frame_buffer->linesize, 0, ctx->codec_context->height, ctx->rgb_frame_buffer, ctx->rgb_frame_buffer->linesize);
...
fas_error_type private_convert_to_gray8 (fas_context_ref_type ctx)
{
    ...
    sws_scale(convert_context, ctx->frame_buffer->data, ctx->frame_buffer->linesize, 0, ctx->codec_context->height, ctx->gray8_frame_buffer, ctx->gray8_frame_buffer->linesize);
